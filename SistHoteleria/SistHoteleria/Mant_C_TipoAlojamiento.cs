...
{
    public partial class Mant_C_TipoAlojamiento : Form
    {
        string sql = "";
        public string Id = "";
        public string aa_Prov = "";
        string aa_Modo = "";
        public Mant_C_TipoAlojamiento(string ii_modo)
        {
            InitializeComponent();
            CB_Filtro.Items.Add(" ");
        ...
        void Modificar()
        {
            ...
            int i = Fila_Actual();
            Id = DG_Datos.Rows[i].Cells[0].Value.ToString().Trim();
            Clases.ETipoAlojamiento ii_ETipoAlojamiento = funciones.Lee_TipoAlojamiento(Id);
            Mant_TipoAlojamiento form = new Mant_TipoAlojamiento("m", ii_ETipoAlojamiento);
            if (form.ShowDialog() == DialogResult.OK)
                Lee_Datos();

        }
        private void Mant_C_TipoAlojamiento_Load(object sender, EventArgs e)
        {
            Lee_Datos();
        }
        void Lee_Datos()
        {
            ...
            if (Count > 0)
            {
                for (int i = 0; i < Count; i++)
                {
                    ...
                    ii_row.Cells[0].Value = DS.Tables[0].Rows[i]["id_t_alojamiento"].ToString().Trim();
                    ii_row.Cells[1].Value = DS.Tables[0].Rows[i]["descr_t_alojamiento"].ToString().Trim();
                    ii_row.Cells[2].Value = DS.Tables[0].Rows[i]["costo_t_alojamiento"].ToString().Trim();
        ...
        {
            Form1.Imprime(sql, "RP_LTipoAlojamiento", false);
        }
    }
}
