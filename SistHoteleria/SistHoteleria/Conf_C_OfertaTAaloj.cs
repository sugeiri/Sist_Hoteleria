...
{
    public partial class Conf_C_OfertaTAaloj : Form
    {
        ...
        public Conf_C_OfertaTAaloj(string ii_modo)
        {
            InitializeComponent();
            aa_modo = ii_modo;
        }
        public Conf_C_OfertaTAaloj(string ii_sql, string ii_modo)
        {
            InitializeComponent();
            aa_modo = ii_modo;
            sql = ii_sql;
        ...
        private void Conf_C_OfertaTAaloj_Load(object sender, EventArgs e)
        {
        ...
        void Lee_Datos()
        {
             ...
             DG_Datos.Rows.Clear();
            string Error = "";
            string sql = "select id_oferta_det01,descr_oferta,id_t_aloj_det01,descr_t_alojamiento,descuento_det01 " +
                                " from oferta_det_01,Oferta,tipo_alojamiento  " +
                                " where id_oferta_det01=id_oferta and " +
                                "   id_t_aloj_det01=id_t_alojamiento  ";
            ...
            if (Count > 0)
            {
                ...
                for (int i = 0; i < Count; i++)
                {
                    ...
                    ii_row.Cells[0].Value = DS.Tables[0].Rows[i]["id_t_aloj_det01"].ToString().Trim();
                    ii_row.Cells[1].Value = DS.Tables[0].Rows[i]["descr_t_alojamiento"].ToString().Trim();
                    ii_row.Cells[2].Value = DS.Tables[0].Rows[i]["id_oferta_det01"].ToString().Trim();
                    ii_row.Cells[3].Value = DS.Tables[0].Rows[i]["descr_oferta"].ToString().Trim();
                    ii_row.Cells[4].Value = DS.Tables[0].Rows[i]["descuento_det01"].ToString().Trim();
