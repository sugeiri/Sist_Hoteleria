...
namespace SistHoteleria
{
    public partial class Mant_C_TipoHabitacion : Form
    {
        ...
        public Mant_C_TipoHabitacion(string ii_modo)
        {
            InitializeComponent();
            CB_Filtro.Items.Add(" ");
            CB_Filtro.Items.Add("Codigo");
        ...
        void Modificar()
        {
            ...
            int i = Fila_Actual();
            Id = DG_Datos.Rows[i].Cells[0].Value.ToString().Trim();
            Clases.ETHabitacion ii_THabitacion = funciones.Lee_TipoHabitacion(Id);
            Mant_TipoHabitacion form = new Mant_TipoHabitacion("m", ii_THabitacion);
            if (form.ShowDialog() == DialogResult.OK)
                Lee_Datos();

        }
        private void Mant_C_TipoHabitacion_Load(object sender, EventArgs e)
        {
            Lee_Datos();
        }
        void Lee_Datos()
        {
            ...
            if (Count > 0)
            {
                for (int i = 0; i < Count; i++)
                {
                    ...
                    ii_row.Cells[0].Value = DS.Tables[0].Rows[i]["id_t_hab"].ToString().Trim();
                    ii_row.Cells[1].Value = DS.Tables[0].Rows[i]["descr_t_hab"].ToString().Trim();
                    ii_row.Cells[2].Value = DS.Tables[0].Rows[i]["capacidad_hab"].ToString().Trim();
                    ii_row.Cells[3].Value = DS.Tables[0].Rows[i]["costo_hab"].ToString().Trim();
