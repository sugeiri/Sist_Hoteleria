...
{
    public partial class Mant_C_Oferta : Form
    {
        public string Id = "";
        public string aa_Prov = "";
        string aa_modo = "";
        public Mant_C_Oferta(string Provincia,string modo)
        {
            InitializeComponent();
            CB_Filtro.Items.Add(" ");
            CB_Filtro.Items.Add("Codigo");
            CB_Filtro.Items.Add("Descripcion");
            CB_Filtro.Items.Add("Temporada");
            aa_Prov = Provincia;
            aa_modo = modo;
        }
        ...
        void Modificar()
        {
            ...
            int i = Fila_Actual();
            Id = DG_Datos.Rows[i].Cells[0].Value.ToString().Trim();
            Clases.EOferta ii_Of = funciones.Lee_Oferta(Id);
            Mant_Oferta form = new Mant_Oferta("m", ii_Of);
            if (form.ShowDialog() == DialogResult.OK)
                Lee_Datos();
        }
        private void Mant_C_Oferta_Load(object sender, EventArgs e)
        {
            Lee_Datos();
        }
        void Lee_Datos()
        {
            ...
            {
                sql = " Select id_oferta, descr_oferta, id_temp_oferta, descr_temporada from oferta,Temporada " +
                     " where  estado_oferta = 'A' and id_temp_oferta ='" + aa_Prov + "'" +
                     " and id_temp_oferta = id_temporada";
            }
            else
            {
                sql = " Select id_oferta, descr_oferta, id_temp_oferta, descr_temporada  from oferta,Temporada " +
                   " where  estado_oferta = 'A' and id_temp_oferta = id_temporada";
            ...
            if (Count > 0)
            {
                for (int i = 0; i < Count; i++)
                {
                    ...
                    ii_row.Cells[0].Value = DS.Tables[0].Rows[i]["id_oferta"].ToString().Trim();
                    ii_row.Cells[1].Value = DS.Tables[0].Rows[i]["descr_oferta"].ToString().Trim();
                    ii_row.Cells[2].Value = DS.Tables[0].Rows[i]["descr_temporada"].ToString().Trim();
