...
{
    public partial class Conf_C_OfertaxRango : Form
    {
        ...
        public Conf_C_OfertaxRango(string ii_modo)
        {
            InitializeComponent();
            aa_modo = ii_modo;
        }
        public Conf_C_OfertaxRango(string ii_sql, string ii_modo)
        {
            InitializeComponent();
            aa_modo = ii_modo;
            sql = ii_sql;
        ...
        void Modificar()
        {
            ...
            Conf_OfertaxCantAlojamiento mm = new Conf_OfertaxCantAlojamiento("m", Id);
            mm.ShowDialog();
            Lee_Datos();
            }
            else
        ...
        private void Conf_C_OfertaxRango_Load(object sender, EventArgs e)
        {
        ...
        void Lee_Datos()
        {
             ...
             DG_Datos.Rows.Clear();
            string Error = "";
            string sql = "select id_oferta_det03,descr_oferta,c_ini_aloj_det03,c_fin_aloj_det03,descuento_det03 " +
                                " from oferta_det_03,Oferta " +
                                " where id_oferta_det03=id_oferta  ";
            ...
            if (Count > 0)
            {
                ...
                for (int i = 0; i < Count; i++)
                {
                    ...
                    ii_row.Cells[0].Value = DS.Tables[0].Rows[i]["c_ini_aloj_det03"].ToString().Trim();
                    ii_row.Cells[1].Value = DS.Tables[0].Rows[i]["c_fin_aloj_det03"].ToString().Trim();
                    ii_row.Cells[2].Value = DS.Tables[0].Rows[i]["id_oferta_det03"].ToString().Trim();
                    ii_row.Cells[3].Value = DS.Tables[0].Rows[i]["descr_oferta"].ToString().Trim();
                    ii_row.Cells[4].Value = DS.Tables[0].Rows[i]["descuento_det03"].ToString().Trim();
