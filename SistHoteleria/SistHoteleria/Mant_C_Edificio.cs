...
{
    public partial class Mant_C_Edificio : Form
    {
        public string Id = "";
        ...
        void Modificar()
        {
            ...
            Mant_Edificio form = new Mant_Edificio("m", ii_Edificio);
            if (form.ShowDialog() == DialogResult.OK)
                Lee_Datos();

        }
        private void Mant_C_Edificio_Load(object sender, EventArgs e)
        {
            Lee_Datos();
        }
        void Lee_Datos()
        {
            ...
            if (Count > 0)
            {
                for (int i = 0; i < Count; i++)
                {
                    ...
                    ii_row.Cells[0].Value = DS.Tables[0].Rows[i]["id_edificio"].ToString().Trim();
                    ii_row.Cells[1].Value = DS.Tables[0].Rows[i]["descr_edificio"].ToString().Trim();
                    ii_row.Cells[2].Value = DS.Tables[0].Rows[i]["cant_nivel_edificio"].ToString().Trim();
