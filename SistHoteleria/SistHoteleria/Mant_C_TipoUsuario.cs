...
{
    public partial class Mant_C_TipoUsuario : Form
    {
        public string Id = "";
        public string aa_Prov = "";
        string aa_Modo = "";
        public Mant_C_TipoUsuario()
        {
            InitializeComponent();
            CB_Filtro.Items.Add(" ");
            CB_Filtro.Items.Add("Codigo");
            CB_Filtro.Items.Add("Descripcion");
            CB_Filtro.Items.Add("Nivel");

        }
        public Mant_C_TipoUsuario(string ii_modo)
        {
            InitializeComponent();
            CB_Filtro.Items.Add(" ");
            CB_Filtro.Items.Add("Codigo");
        ...
        void Modificar()
        {
            ...
            int i = Fila_Actual();
            Id = DG_Datos.Rows[i].Cells[0].Value.ToString().Trim();
            Clases.ETipoUsuario ii_TUsuario = funciones.Lee_TipoUsuario(Id);
            Mant_TipoUsuario form = new Mant_TipoUsuario("m", ii_TUsuario);
            if (form.ShowDialog() == DialogResult.OK)
                Lee_Datos();

        }
        private void Mant_C_TipoUsuario_Load(object sender, EventArgs e)
        {
            Lee_Datos();
        }
        void Lee_Datos()
        {
            ...
            if (Count > 0)
            {
                for (int i = 0; i < Count; i++)
                {
                    ...
                    ii_row.Cells[0].Value = DS.Tables[0].Rows[i]["id_T_Usuario"].ToString().Trim();
                    ii_row.Cells[1].Value = DS.Tables[0].Rows[i]["Descripcion_T_Usuario"].ToString().Trim();
                    ii_row.Cells[2].Value = DS.Tables[0].Rows[i]["Nivel_Acceso_T_Usuario"].ToString().Trim();
